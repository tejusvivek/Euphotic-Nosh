...
def processLine(recipelinelist):
    ...
    if (recipelinelist[0] == "vegmap"):
        ...
        if (recipelinelist[1] == "start"):
            ...
            time.sleep(4)
        elif (recipelinelist[1] == "stop"):
            writeNumber("stove_off", 0)
        elif (recipelinelist[1] == "heat"):
            writeNumber("stove_level", int(recipelinelist[2]))

    elif (recipelinelist[0] == "wait"):
        time.sleep(float(recipelinelist[1]) - 1)
    ...
    # time.sleep(float(recipelinelist[2]))
        ...
        while (tempreading <= float(recipelinelist[1])):
            ...
            f.write(
                "data time is " + str(datetime.now()) + str(" : the temperature is :") + str(tempreading) + str("\n"))
